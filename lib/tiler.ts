...
export default abstract class Tiler<T, TRaw = T> {
    tile: Tile;
    url: string;
    ...
    constructor(url: string) {
        ...
        this.dataTiles = new Map<string, DataTile<T>>();
    }
     ...
     * @return {Promise<DataTile<T>[]>} a Promise resolving with an array of the newly-downloaded data tiles (only new tiles; existing ones are not returned)
     */
    async update(pos: EastNorth): Promise<DataTile<T>[]> {
        const loadedData: DataTile<T>[] = [];
     ...
     * @return {Promise<DataTile<T>[]>} a Promise resolving with an array of the newly-downloaded data tiles (only new tiles; existing ones are not returned)
     */
    async updateByLonLat(lonLat: LonLat): Promise<DataTile<T>[]> {
        return this.update(this.lonLatToSphMerc(lonLat));
    }

    /**
     * Obtain the current tiles surrounding the last-requested position.
     * @return {DataTile<T>[]} array of 9 tiles, including the current tile and the 8 surrounding it.
     */
    getCurrentTiles(): DataTile<T>[] {
        const tiles: DataTile<T>[] = [];
        ...
        for (let ix = 0; ix < tilesX.length; ix++) {
            for (let iy = 0; iy < tilesY.length; iy++) {
                ...
                if (data !== undefined) {
                    tiles.push({ tile, data: data.data });
                }
            }
        }
        return tiles;
     ...
     * @return {Promise<TRaw>} a Promise resolving with arbitrary data. The raw data type
     * might not be the same as the tiled type, e.g. in the DemTiler, the raw data is RawPngData but the
     * tiled data type is DEM.
     */
    abstract readTile(url: string): Promise<TRaw>;
     ...
     * @return {Promise<DataTile<T> | null>} Promise resolving with the given Tile, or null if it couldn't be loaded
     */
    async getData(sphMercPos: EastNorth, z: number = 13): Promise<DataTile<T> | null> {
        await this.update(sphMercPos);
        const thisTile = this.sphMerc.getTile(sphMercPos, z);
        return this.dataTiles.get(`${z}/${thisTile.x}/${thisTile.y}`) ?? null;
    }
     ...
     * @param {TRaw} data - the raw downloaded data.
     * @return {DataTile<T>} the DataTile pairing the Tile and the data in the format we want.
     */
    rawTileToStoredTile(tile: Tile, data: TRaw): DataTile<T> {
        return { tile, data: data as any as T };
    }
}
