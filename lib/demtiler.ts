 ...
 * @class
 */
export default class DemTiler extends Tiler<DEM, RawPngData> {
     ...
     * @param {string} url - the Terrarium server URL.
     * @return {Promise<RawPngData>} a Promise resolving with raw Terrarium data.
     */
    ...
    getElevation(sphMercPos: EastNorth): number | null {
        ...
        if (indexedTile) {
            return indexedTile.data.getElevation(sphMercPos.e, sphMercPos.n);
        }
        return null;
    }
     ...
     * @param {RawPngData} data - the raw downloaded data.
     * @return {DataTile<DEM>} the DataTile pairing the Tile and the DEM data as a DEM object. 
     */
    rawTileToStoredTile(tile: Tile, data: RawPngData): DataTile<DEM> {
